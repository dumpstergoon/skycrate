	...
	*/
	...
	const template = (name, type, static, definition, ...templates) => {
		...
		if (type === ELEMENT) {
			...
			template = values => apply(document.createElement(name), concat(defaults, values));
		...
		let element = class extends HTMLElement {
			...
			connectedCallback() {
				...
				rows(_attributes).forEach(([name, attribute]) => {
					...
					attribute.render(this, value, value);
				});
			...
			render(ts = -1) {
				...
				rows(_attributes).forEach(([name, attribute]) => {
					...
					if (change) {
						attribute.render(this, change.from, change.to);
						this.dispatch(`@${name}`, change, false, false);
					}
				});
	...
	const math = {
		random: extend((a = 0, b = 1) => {
			...
			item: list => list[math.random.range(0, list.length - 1)],
